...
internal_search = Agent(
    ...
    tools=[FileTools()],
    description="""
A data analyst agent that retrieves and summarizes historical client meeting records from the internal database ('Data/data.json').
...
""",
    instructions="""
Only respond if the internal search agent confirms: "CLIENT NOT FOUND".
...
financial_data_agent = Agent(
    ...
    description="""
A financial analyst expert agent responsible for researching specific investment opportunities based on client context. This agent can extract financial data, summarize earnings calls, and provide balanced investment recommendations.

It uses insights from the client's research and combines them with real-time financial data and qualitative sources to support decision-making.
...
# [Stock Name] Overview**
[2-3 sentences summarizing the company core business and differentiation.]
...
**Market Demand**: [Highlight growth sectors/trends.]  
...
- Replace all smart punctuation like curly quotes (", ", ', ') with straight ASCII characters (", ', ').
...
financial_data_agent.print_response("Summarize what Oklo does and provide the pros and cons of investing into it based on the earnings call Q2 2024", stream=True, markdown=True)
...
recommendation_agent = Agent(
    ...
    description="""
A financial advisory agent responsible for issuing structured investment recommendations based on prior financial analysis. It reads data from 'financial_data.md' and produces a detailed recommendation report using a fixed template, including both qualitative insights and quantitative metrics.
""",
    instructions="""
...
**Rationale**: [2-3 sentence rationale for the rating]
...
| P/E Ratio          | XX.XX       |
| Volume             | X,XXX,XXX   |
| Volatility (Beta)  | X.XX        |

**Company Details**:

| Name       | Rating | Target Price | ISIN       | Last Price | Currency | Sector  | Sector Rating |
|------------|--------|--------------|------------|------------|----------|---------|---------------|
| [Company]  | [Rating] | [$XX.XX]   | [ISIN]     | $XX.XX     | [Ccy]    | [Sector] | [Sector Rating] |
